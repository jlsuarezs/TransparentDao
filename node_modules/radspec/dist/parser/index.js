...
expression=this.comparison(expression?[expression]:[]);while(!this.eof()&&!this.matches("RIGHT_PAREN"));return this.eof()&&this.report("Unterminated grouping"),{type:"GroupedExpression",body:expression}}this.report(`Unknown token "${this.consume().type}"`)}/**
   ...
   * @param  {Array<Node>} astBody Subtree of AST being walked
   * @return {Array<Node>}
   */functionInputs(astBody){const inputs=[];for(;!this.eof()&&!this.matches("RIGHT_PAREN");){const input=this.comparison(astBody);// Break if the next character is not a comma or a right parenthesis
// If this is true, then we are specifying more parameters without
// delimiting them using comma.
if(input.type?this.matches("COLON")&&this.report(`Unexpected type (already inferred type of parameter)`):input.type=this.type(),inputs.push(input),!this.matches("COMMA")&&"RIGHT_PAREN"!==this.peek().type)break}return inputs}/**
   ...
   * @param {string} error
   * @return {void}
   */report(error){this.state=PARSER_STATE.ERROR,console.error(`Error (${this.cursor}): ${error}`)}}module.exports={Parser,/**
