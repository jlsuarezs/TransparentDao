"use strict";var _interopRequireDefault=require("@babel/runtime/helpers/interopRequireDefault"),_objectSpread2=_interopRequireDefault(require("@babel/runtime/helpers/objectSpread"));/**
 ...
 */ /**
 * @module radspec
 */const ABI=require("web3-eth-abi"),scanner=require("./scanner"),parser=require("./parser"),evaluator=require("./evaluator");/**
 ...
 */function evaluateRaw(source,bindings,evaluatorOptions){return scanner.scan(source).then(parser.parse).then(ast=>evaluator.evaluate(ast,bindings,evaluatorOptions))}/**
 ...
 * @return {Promise<string>} The result of the evaluation
 */function evaluate(source,call,options={}){// Get method ID
const methodId=call.transaction.data.substr(0,10),method=call.abi.find(abi=>"function"===abi.type&&methodId===ABI.encodeFunctionSignature(abi)),parameterValues=ABI.decodeParameters(method.inputs,"0x"+call.transaction.data.substr(10)),parameters=method.inputs.reduce((parameters,input)=>Object.assign(parameters,{[input.name]:{type:input.type,value:parameterValues[input.name]}}),{});// Find method ABI
// Evaluate expression with bindings from
// the transaction data
return evaluateRaw(source,parameters,(0,_objectSpread2.default)({},options,{to:call.transaction.to}))}module.exports={scan:scanner.scan,parse:parser.parse,evaluateRaw,evaluate};
